...
@section('content')
    ...
    <form method="GET" action="{{ route('search_item') }}">
        <input class="form-control me-2" style="margin: 8px auto;" type="search" id="item"
               name="item" placeholder="Поиск" aria-label="Search">
    ...
    @if(count($finances))
        ...
        {{ $finances->appends(['item' => request()->item])->links() }}
    @else
